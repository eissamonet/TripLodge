...
const HotelReg = () => {
  ...
  const {setShowHotelReg, axiosInstance, getToken, setIsOwner} = useAppContext()
      ...
      <form onSubmit={onSubmitHandler} onClick={(e)=> e.stopPropagation()} className='flex bg-white rounded-xl max-w-4xl max-md:mx-2'>
        ...
        <div className='relative flex flex-col items-center md:w-1/2 p-8 md:p-10'>
          ...
          <div className='w-full mt-4'>
            ...
            <input onChange={(e)=> setName(e.target.value)} value={name} id='name' type='text' placeholder='Type Here' className='border border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500 font-light'
            required />
          </div>
          {/* contact number */}
          <div className='w-full mt-4'>
          ...
          <div className='w-full mt-4 max-w-60 mr-auto '>
            ...
            <select onChange={(e)=> setCity(e.target.value)} value={city} id='city' className='border border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500 font-light cursor-pointer' required>
